...
import { useDeleteBulletin } from "@/hooks/useDeleteBulletin";

export default function BulletinListPage() {
  const { data, isLoading, isError, error, refetch, isFetching } = useBulletinList();
  const { mutate: deleteBulletin, isPending: isDeleting, variables: deletingId } =
    useDeleteBulletin();
              ...
              >
                <div>
                  ...
                  <p className="text-label-sm text-muted-foreground">
                    이미지 {item.image_urls.length}장
                  </p>
                </div>
                <div className="flex shrink-0 gap-2">
                  <Link
                    href={`/admin/bulletin/${item.id}`}
                    aria-label="수정"
