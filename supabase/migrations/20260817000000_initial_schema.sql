-- ============================================================
-- 소망가교 초기 스키마
-- 테이블: members, family_members, devotions, devotion_notes,
--        prayer_requests, prayer_reactions, events, songs, daily_songs,
--        sermons, sermon_songs, bible_plans, plan_days, member_plans,
--        reading_logs, push_subscriptions
-- ============================================================
...
-- ============================================================
-- 1. members : 교인 (auth.users 1:1 연동)
-- ============================================================
...
-- auth.users 가입 시 members 행을 자동 생성 (기본 role=member, status=pending)
...
-- role/status는 관리자만 변경 가능하도록 방어 (RLS만으로는 컬럼 단위 보호가 어려움)
...
begin
  if not public.is_admin() then
    if new.role is distinct from old.role or new.status is distinct from old.status then
      raise exception 'role/status는 관리자만 변경할 수 있습니다.';
    end if;
  end if;
  return new;
end;
...
create trigger members_prevent_privilege_escalation
before update on members
for each row execute function prevent_member_privilege_escalation();

-- ============================================================
-- 2. family_members : 교인의 가족 (생일 알림 등에 사용)
-- ============================================================
...
-- ============================================================
-- 6. prayer_reactions : "함께 기도해요" 참여 (prayingCount 집계용)
-- ============================================================
...
-- ============================================================
-- 10. sermons : 설교 (가교모임 설교요약/나눔질문 포함)
-- ============================================================
...
-- ============================================================
-- 11. sermon_songs : 설교(모임)에 연결된 찬양 악보
-- ============================================================
...
-- ============================================================
-- 14. member_plans : 교인의 플랜 등록/진행 상태
-- ============================================================
...
-- ============================================================
-- 16. push_subscriptions : 웹 푸시 구독 정보
-- ============================================================
...
-- role/status 변경은 관리자만 (트리거로 이중 방어), 가입 시 행 생성은 auth 트리거가 처리
-- ============================================================
create policy "members_select_approved_or_self_or_admin" on members
...
-- ============================================================
-- RLS 정책 : family_members (본인 가족만 관리, 관리자는 전체)
-- ============================================================
create policy "family_members_select_own_or_admin" on family_members
for select to authenticated
...
create policy "family_members_update_own_or_admin" on family_members
for update to authenticated
using (member_id = auth.uid() or is_admin())
with check (member_id = auth.uid() or is_admin());

create policy "family_members_delete_own_or_admin" on family_members
for delete to authenticated
using (member_id = auth.uid() or is_admin());
